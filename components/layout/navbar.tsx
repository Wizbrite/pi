...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-transparent backdrop-blur-md">
        ...
        <div className="hidden items-center gap-3 md:flex">
          {isAuthenticated ? (
            ...
            <>
              <Link
                href="/login"
                className="rounded-lg px-5 py-2 text-sm font-semibold text-white transition-colors hover:text-primary"
              >
                Log In
              </Link>
              <Link
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="border-t border-violet-900/30 bg-[#06071a]/98 backdrop-blur-md md:hidden">
          <div className="space-y-1 px-4 py-4">
            ...
            <div className="mt-4 flex flex-col gap-2 border-t border-border pt-4">
              <Link
                href="/login"
                onClick={() => setMobileOpen(false)}
                className="rounded-lg px-3 py-2.5 text-center text-sm font-semibold text-white transition-colors hover:bg-muted"
              >
                Log In
              </Link>
              <Link
