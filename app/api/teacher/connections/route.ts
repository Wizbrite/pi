...
export async function GET() {
  try {
    ...
    const connections = await teacherConnectionService.getTeacherConnections(userId);
...
export async function POST(request: Request) {
  try {
    ...
    const connection = await teacherConnectionService.sendTeacherRequest(userId, studentEmail, message);
